...
N_CLS = 200 # 200 classes for Tiny ImageNet

os.makedirs(args.save_folder)

name_log_txt = time_stamp + str(randint(0, N_CLS)) + args.name
...
try:
  import wandb
  if args.wandb_name and args.wandb_name != 'default':
    wandb.init(
      project='complearn',
      name=args.wandb_name)
  else:
    wandb.init(project='complearn')
  USE_WANDB = True
except:
  USE_WANDB = False
...
def main():
    ...
    train_loader = get_tinyImgNet_train_loader(
    ...
    val_loader = get_tinyImgNet_test_loader(
      batch_size=args.batch_size,
      shuffle=False,
    )
...
def validate(loader, model, criterion, n):
    ...
    if args.ensemble:
        ...
        total_out = torch.zeros([total, N_CLS])
